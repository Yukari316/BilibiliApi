...
namespace BilibiliApi.Dynamic.Models
{
    ...
    public abstract class Dynamic : BaseApiInfo
    {
        ...
        protected Dynamic(JToken apiResponse, int index) : base(apiResponse)
        {
            ...
            var emojiData = new Dictionary<string, string>();
            //判断是否存在emoji
            var emojis = cardJObject["display"]?["emoji_info"]?["emoji_details"]?.ToArray();
            if (emojis        != null &&
                emojis.Length != 0)
            {
                int unknownCount = 0;
        ...
        /// 初始化父数据
        /// </summary>
        protected Dynamic(JToken cardData) : base(cardData)
        {
            if (base.Code != 0) return;
            var cardJObject = cardData["data"]?["card"] ?? new JObject();

            //避免空json
            if (!cardJObject.HasValues) return;
        ...
        internal static CardType GetCardType(JToken sourceData, int index)
        {
            ...
            var typeCode = Convert.ToInt32(sourceData["data"]?["cards"]?[realIndex]?["desc"]?["type"] ?? -1);
            return Enum.IsDefined(typeof(CardType), typeCode)
                ? (CardType) typeCode
                : CardType.Unknown;
        }
        ...
        /// 获取单独一条动态的类型
        /// </summary>
        internal static CardType GetCardType(JToken sourceData)
        {
            var typeCode = Convert.ToInt32(sourceData["data"]?["card"]?["desc"]?["type"] ?? -1);
            return Enum.IsDefined(typeof(CardType), typeCode)
                ? (CardType) typeCode
                : CardType.Unknown;
        }
